...
function Cards({data,title}) {
  return (
    <div className='flex flex-wrap w-full px-[5%] bg-[#1F1E24] mt-8'>
        {data.map((c,i)=> (
        ...
        {c.vote_average && ( <div className='absolute right-[-5%] bottom-[35%] rounded-full text-xl font-semibold bg-yellow-600 text-white w-[7vh] h-[7vh] flex justify-center items-center'>{(c.vote_average*10).toFixed()} <sup>%</sup></div>)}
        </Link>))}
    </div>
  )
}
